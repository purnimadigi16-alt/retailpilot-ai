...
This document presents the complete 50-test-case Quality Assurance specification for RetailPilot AI. All 50 test cases are automated and verifiable via `npm test` or the in-app interactive `/qa-matrix` explorer.
...
- **FT-02: Split Payments Equal Total Invoice** — Verified multi-payment split (Cash + Card + UPI + Points) sum equals invoice total.
...
- **FT-05: Sales Tax Calculation Precision** — Verified 8% sales tax computed accurately on discounted taxable subtotal.
...
- **FT-09: Customer Loyalty Point Accrual** — Verified customer earns 1 loyalty point per $10 spent at checkout.
- **FT-10: Loyalty Point Redemption at POS** — Verified redeeming points deducts from customer loyalty point balance.
...
- **FT-13: Store Operating Expense Categorization** — Verified expenses categorized into Rent, Utilities, Salaries, Marketing, Maintenance.
- **FT-14: Cost of Goods Sold (COGS) Calculation** — Verified COGS calculated from unit cost price * quantity sold.
- **FT-15: Gross Margin % Computation** — Verified Gross Margin = ((Gross Sales - COGS) / Gross Sales) * 100.
- **FT-16: Net Profit Margin Computation** — Verified Net Profit = Gross Profit - Operating Expenses.
- **FT-17: Multi-Store Outlet Scoping** — Verified queries filter by specific store outlet or aggregate across tenant organization.
- **FT-18: Product SKU Uniqueness Per Tenant** — Verified database unique constraint on (organization_id, sku).
- **FT-19: Barcode Exact SKU Resolution** — Verified barcode scanner input resolves exact matching product title and price.
...
- **FT-24: Customer Profile Linked to Sale Invoice** — Verified customer foreign key association on sales invoices.
...
- **API-02: GET /api/products Organization Filter** — Verified 100% returned items match requested tenant ID.
...
- **API-09: Sub-150ms Analytics Query Latency** — Verified /api/analytics response latency completes within target threshold.
...
- **SEC-01: Cross-Tenant Product Isolation (Org A vs Org B)** — Verified Org A cannot view or manipulate Org B products.
...
- **UI-02: Thermal Invoice Print CSS (@media print)** — Verified receipt print styling formats for 80mm thermal receipt rolls.
